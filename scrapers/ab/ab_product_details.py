...
# Fetches Information on a specific product
def fetchproductDetails(code, session):
    url = "https://www.ab.gr/api/v1/"

    params = {
        "operationName": "ProductDetails",
        "variables": json.dumps({"productCode":f"{code}","lang":"gr"}, separators=(',', ':')),
        "extensions": json.dumps(
        {"persistedQuery":{"version":1,"sha256Hash":"56c710d5a8fbfd875435defa32acce0497637a2987eeb9be0d4a117ea33bdb2a"}}
        , separators=(',', ':'))
    }
    ...
    output = fetchParseDetailsResponse(code, response)
    return output


# This function is used to convert the response to the desired format
def fetchParseDetailsResponse(code, response):
    ...
    mappings = {
        ...
        'Συνολικά λιπαρά': 'fat',
        'εκ των οποίων κορεσμένα λιπαρά': 'saturated_fat',
        'Υδατάνθρακες': 'carbs',
        'Σάκχαρα': 'sugars',
        'Πρωτεΐνες': 'proteins',
        'Αλάτι': 'salt',
